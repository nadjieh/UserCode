/* 
 * File:   EventShapeVariables.h
 * Author: ajafari
 *
 * Created on September 15, 2010, 3:08 PM
 */
//http://cepa.fnal.gov/psm/simulation/mcgen/lund/pythia_manual/pythia6.3/pythia6301/node213.html
//http://prd.aps.org/pdf/PRD/v18/i9/p3378_1
#ifndef _EVENTSHAPEVARIABLES_H
#define	_EVENTSHAPEVARIABLES_H
#include "TMath.h"
...
public:
    EventShapeVariables(TopEvent top):Top(top){//using jets
        inputVectors_.clear();
        //LepB
        inputVectors_.push_back(math::XYZVector(Top.getLepBCand().getP4().X(),
        Top.getLepBCand().getP4().Y(),Top.getLepBCand().getP4().Z()));
        //HadB
        inputVectors_.push_back(math::XYZVector(Top.getHadBCand().getP4().X(),
        Top.getHadBCand().getP4().Y(),Top.getHadBCand().getP4().Z()));
        //Q
        inputVectors_.push_back(math::XYZVector(Top.getQ1().getP4().X(),
        Top.getQ1().getP4().Y(),Top.getQ1().getP4().Z()));
        //QBar
        inputVectors_.push_back(math::XYZVector(Top.getQ2().getP4().X(),
        Top.getQ2().getP4().Y(),Top.getQ2().getP4().Z()));
    };
    virtual ~EventShapeVariables(){};
    ...
    double isotropy(const unsigned int& numberOfSteps) const
     {
...
double circularity(const unsigned int& numberOfSteps) const
{
  const double deltaPhi=2*TMath::Pi()/numberOfSteps;
