...
//              isTtBar = true;
          ...
          in ="/home/ajafari/rootfiles/TopTrees/7TeV/July10/SystematicSamples/FullSelection/FS_"+in;
...
//    /*ExtJets*/ExtJets);
    DataAnalysis * myDataAnalysis = new DataAnalysis(("FdataData_"+anaName), 50., 160.,160.,280.,
...
//            Fullanalysis.Analyze(pracEvt,weight);
            Fullanalysis.Analyze(pracEvt,1);
//            if(i == 100)
//                break;
        }
    }
    double Fmc = myMCAnalysis->getF();
    double Fdata_ = myDataAnalysis->getF();
    if(Fdata != -1){
        Fullanalysis.setAnalysisNonTrivialF(1,Fdata);
        Fdata_ = Fdata;
    }
    Fullanalysis.End();
    cout<<"\t\t--------------------------------"<<endl;
    cout<<"\t\tFmc = "<<Fmc<<endl;
    cout<<"\t\tFdata = "<<Fdata_<<endl;
