...
//	run_br->SetAddress(&runInfos);
	...
	event_br->SetAddress(&event);
        ...
        ge_br = (TBranch *) eventTree->GetBranch("GenEvent");
        GEs = new TClonesArray("TopTree::TRootGenEvent", 0);
        ge_br->SetAddress(&GEs);
...
//                    PassedBtagSelection.Fill(myEvent_tmp);
            ...
            topGE = (TRootGenEvent*)GEs->At(0);
            if(topGE->isFullHadronic())
                nFullHad++;
            else if(topGE->isFullLeptonic())
                nDiLep++;
            else if(topGE->semiLeptonicChannel() == topGE->kMuon)
                nSemiMu++;
            else if(topGE->semiLeptonicChannel() == topGE->kTau)
                nSemiTau++;
        }
}
