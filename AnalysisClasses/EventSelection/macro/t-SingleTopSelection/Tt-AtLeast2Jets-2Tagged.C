...
bool isTtLike(Event evt, double bdisc = 3.41, int nTagged = 1, int nJet = 3){
    if(evt.GPFJets.size() < nJet)
        return false;
    int nTag = 0;
    int nJets = 0;
...
//    sleep(60);
    ...
    std::string HLTname="HLT_IsoMu17_v*";//"HLT_Mu17_eta2p1_CentralJet30_BTagIP_v1";//
        ...
        if (arg_fth == "out") {
          ...
          sin_outFileName_FullSelection = string(out);
...
//    cout<<doJES<<endl;
    ...
    for(unsigned int fNumber = 0; fNumber<sin_inputFileNames.size(); fNumber++){
        ...
        if(fillTree ){
            stringstream s("");
//            s<<sin_outFileName_FullSelection<<"_"<<(fNumber+1)<<".root";
            s<<sin_outFileName_FullSelection<<"_"<<(fNumber+1)<<".root";
...
//                int trigID = hltInfo.hltPath("");
            ...
            if(TopPairtemplate && !isTtLike(myEvent_tmp,3.41,2,2))
                continue;
            if(fillTree){
                if(saveTypeIMET)
