...
TH2* DiagonalMaker(TH2* input, unsigned int j = 0) {
    ...
    delete h1;
    stringstream s;
    s << j << "_" << output->GetName();
    output->SetName(s.str().c_str());
    return output;
}
    ...
    for (int nPEX = 0; nPEX < 300; nPEX++) {
        ...
        for (int i = 0; i < nFSteps; i++) {
            ...
            if (!is2D) {
                ...
                signalIID.clear();
                for (unsigned int iTwoD = 0; iTwoD < tmpVec2D.size(); iTwoD++) {
                    signalIID.push_back(tmpVec2D.at(iTwoD));
                }
                std::cout<<"no muon: "<<signalIID.size()<<endl;                
#endif
