...
//                coefficients->Multiply(&(WeightFuncTT.first), 1);
...
TH1* MakeRandomHistogram(TH1* inHist, int nPE, int nDim = 1) {
    TRandom seed;
    ...
    if (nDim == 1) {
        ...
        for (int iBin = 0; iBin < inHist->GetXaxis()->GetNbins(); iBin++) {
            double nBinNew = s.Gaus(inHist->GetBinContent(iBin + 1), inHist->GetBinError(iBin + 1));
