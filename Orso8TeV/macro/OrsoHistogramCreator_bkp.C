...
int main(int argc, char** argv) {
    ...
    for (int f = 1; f < argc; f++) {
        ...
        if (arg_fth == "input") {
            ...
            inputFileNames.push_back(string("~/work/samples/Orso8TeV/oct/" + in + ".root"));
            sample = in;
    ...
    TH2D * nvtx_mtW = new TH2D("nvtxMtW", "nvtxMtW", 300, 0, 300, 300, 0., 300.);
    ...
    for (unsigned int fNumber = 0; fNumber < inputFileNames.size(); fNumber++) {
        ...
        if (string(myMuonTree->fChain->GetName()) == string("Data_2J_1T_QCD_noSyst")) {
            plotFileName = "QCDTemplate" + plotFileName;
        }
        ...
        for (int eventNumber = 0; eventNumber < myMuonTree->fChain->GetEntriesFast(); eventNumber++) {
            ...
            nVtx_cosTheta->Fill(myMuonTree->nVertices, myLeptonicTop.cosThetaStar(), 1);
            ...
            nvtx_mtW->Fill(myMuonTree->nVertices, myMuonTree->GetMET().Pt(), defW);
