#ifndef TOPSINGLELEPTONTRIGGEROBJECTDQM
#define TOPSINGLELEPTONTRIGGEROBJECTDQM
#include <string>
#include <vector>
...
namespace TopSingleLepton {
  ...
  public:
    /// default contructor
    TOMonitorEnsemble(const char* label, const edm::ParameterSet& cfg);
    /// default destructor
    ~TOMonitorEnsemble(){};
    ...
    trigger::TriggerObjectCollection getTriggerObjectCollection(const edm::Handle<edm::TriggerResults> triggerResultsHandle_, const edm::Handle<trigger::TriggerEvent> triggerEventHandle_ , const unsigned int triggerIndex);
    inline void setInitiatedHLTConfig(HLTConfigProvider input){hltConfig_ = input;};
  ...
  private:
    ...
    void triggerBinLabels(std::string channel, const std::string labels);
    /// fill trigger monitoring histograms
    void fill(const edm::Event& event, const edm::TriggerResults& triggerTable, std::string channel, const std::string labels) const;
  ...
  inline void 
  TOMonitorEnsemble::triggerBinLabels(std::string channel, const std::string labels)
  {
//    for(unsigned int idx=0; idx<labels.size(); ++idx){
      hists_[(channel+"Mon_").c_str()]->setBinLabel( 1, "["+monitorPath(labels)+"]", 1);
      hists_[(channel+"Eff_").c_str()]->setBinLabel( 1, "["+selectionPath(labels)+"]|["+monitorPath(labels)+"]", 1);
//    }
  }

  inline void 
  TOMonitorEnsemble::fill(const edm::Event& event, const edm::TriggerResults& triggerTable, std::string channel, const std::string labels) const
  {
//    for(unsigned int idx=0; idx<labels.size(); ++idx){
      if( accept(event, triggerTable, monitorPath(labels)) ){
	fill((channel+"Mon_").c_str(), 0.5 );
	// take care to fill triggerMon_ before evts is being called
	int evts = hists_.find((channel+"Mon_").c_str())->second->getBinContent(1);
	double value = hists_.find((channel+"Eff_").c_str())->second->getBinContent(1);
	fill((channel+"Eff_").c_str(), 0.5, 1./evts*(accept(event, triggerTable, selectionPath(labels))-value));
      }
//    }
  }
...
class TopSingleLeptonTriggerObjectDQM : public edm::EDAnalyzer  {
 public: 
  /// default constructor
  TopSingleLeptonTriggerObjectDQM(const edm::ParameterSet& cfg);
  /// default destructor
  ~TopSingleLeptonTriggerObjectDQM(){
  ...
  HLTConfigProvider getInitiatedHLTConfig()const{return hltConfig_;}  
 private:
 ...
 private:
  ...
  std::map<std::string, std::pair<edm::ParameterSet, TopSingleLepton::TOMonitorEnsemble*> > selection_;
};
